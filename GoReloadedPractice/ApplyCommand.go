...
func ApplyCommand(ref, cmd string, count, commandStarts int) string {
	before := strings.TrimSpace(ref[:commandStarts])
	after := ref[commandStarts:]
...
func removeCommand(text string) string {
	...
	if open != -1 && close != -1 && close > open {
		return strings.TrimSpace(text[:open] + text[close+1:])
	}
	return text
}
